...
async function ensurePlayerIdCookie() {
  try {
    await fetch('/jonsgames/api/scorch2025/init-player', {
      method: 'POST',
      credentials: 'include',
    });
  } catch (err) {
...
async function fetchGames() {
  try {
    const response = await fetch('/jonsgames/api/scorch2025/games', { credentials: 'include' });
    if (!response.ok) {
...
newGameForm.addEventListener('submit', async function (event) {
  ...
  try {
    const response = await fetch('/jonsgames/api/scorch2025/create-game', {
      method: 'POST',
      credentials: 'include',
