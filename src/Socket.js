...
async function ensurePlayerId() {
    ...
    if (!playerId) {
        const response = await fetch('/jonsgames/api/scorch2025/init-player', {
            method: 'POST',
            credentials: 'include'
        });
...
async function initializeSocket() {
    try {
        ...
        socket = io('/', {
            path: '/jonsgames/api/scorch2025/socket.io',
...
const setupSocketEvents = () => {
    // Only set up connection events initially
    // Other event handlers are set up in gameSetup handler after game.init()
...
const setupConnectionEvents = () => {
    ...
    socket.on('gameSetup', (data) => {
        game.init(data);
        // Set up other event handlers AFTER the game is initialized
        // This prevents race conditions where events fire before the scene is ready
        setupGameEvents();
        setupPlayerEvents();
        setupTurnEvents();
        setupCombatEvents();
